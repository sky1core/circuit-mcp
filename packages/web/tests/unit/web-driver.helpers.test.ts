...
describe('WebDriver Helpers', () => {
  ...
  describe('generatePlaywrightTest', () => {
    it('should generate valid Playwright test code', async () => {
      const mockSession = {
        recordedActions: [
          ...
          { type: 'click', selector: '#login-button', timestamp: 1 },
          { type: 'type', selector: '#username', text: 'testuser', timestamp: 2 },
          { type: 'type', selector: '#password', text: 'secret', timestamp: 3 },
          { type: 'key', text: 'Enter', timestamp: 4 },
        ],
      } as Partial<WebSession>;

      const testCode = await driver.generatePlaywrightTest(mockSession);

      expect(testCode).toContain("require('@playwright/test')");
      expect(testCode).toContain("await page.goto('https://example.com')");
      expect(testCode).toContain("await page.click('#login-button')");
      expect(testCode).toContain("await page.fill('#username', 'testuser')");
      expect(testCode).toContain("await page.fill('#password', 'secret')");
      expect(testCode).toContain("await page.keyboard.press('Enter')");
    });

    it('should handle hover actions', async () => {
      const mockSession = {
        recordedActions: [
          { type: 'hover', selector: '.menu-item', timestamp: 0 },
        ],
      } as Partial<WebSession>;

      const testCode = await driver.generatePlaywrightTest(mockSession);

      expect(testCode).toContain("await page.hover('.menu-item')");
    });

    it('should handle select actions', async () => {
      const mockSession = {
        recordedActions: [
          { type: 'select', selector: '#country', text: 'usa', timestamp: 0 },
        ],
      } as Partial<WebSession>;

      const testCode = await driver.generatePlaywrightTest(mockSession);

      expect(testCode).toContain("await page.selectOption('#country', 'usa')");
    });

    it('should escape single quotes in URL', async () => {
      const mockSession = {
        recordedActions: [
          { type: 'navigate', text: "https://example.com/path?q='test'", timestamp: 0 },
        ],
      } as Partial<WebSession>;

      const testCode = await driver.generatePlaywrightTest(mockSession);

      // Should escape quotes to avoid breaking JavaScript string
      expect(testCode).not.toContain("?q='test'");
    ...
    it('should escape single quotes in text', async () => {
      const mockSession = {
        recordedActions: [
          { type: 'type', selector: '#input', text: "it's a test", timestamp: 0 },
        ],
      } as Partial<WebSession>;
      ...
      // Should escape quotes to avoid breaking JavaScript string
      expect(testCode).not.toContain(", 'it's");
      expect(testCode).toContain("\\'");
    });
    ...
    it('should escape single quotes in selector', async () => {
      const mockSession = {
        recordedActions: [
          { type: 'click', selector: "[data-value='test']", timestamp: 0 },
        ],
      } as Partial<WebSession>;
      ...
      // Should escape quotes to avoid breaking JavaScript string
      expect(testCode).not.toContain("('[data-value='test']')");
      expect(testCode).toContain("\\'");
    });
  });
});
